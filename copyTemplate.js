...
const setup = async () => {
	try {
		...
		const { dirAnswer } = await inquirer.createPromptModule()({
			...
			name: "dirAnswer",
			message: chalk.green(
				'Enter destination folder (use "." for current directory):',
			),
		});

		const targetDir = path.resolve(
			dirAnswer === "." ? process.cwd() : path.join(process.cwd(), dirAnswer),
		);
		...
		console.log(
			chalk.green("Project setup complete with all commands run successfully!"),
		);
	} catch (error) {
